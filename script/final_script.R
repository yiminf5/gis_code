...
NYCSubBoroughs <- st_read(here::here("final_project", "data", "raw", "subb", "subb_test2.shp"))
...
NYCBoroughs <- st_read(here::here("final_project", "data", "raw", "nybb_20c", "nybb.shp"))
...
qtm(NYCNeighbor, title = "NYC Neighborhood Tabulation Areas", border = '#addd8e', alpha = 0.5)
qtm(NYCBoroughs)
...
NYCNeighbor  <- NYCNeighbor %>% st_set_crs(4326)
...
tm_shape(NYCBoroughs)+ 
  tm_graticules(col = "grey", alpha=0.2)+
  tm_polygons("BoroName", palette = c("#a6611a", "#dfc27d", "#f5f5f5", "#80cdc1", "#018571"), alpha = 0.85, title = "Borough Names")+
  tm_text("BoroName", xmod=0, ymod=-0.5, size = 0.9, shadow = TRUE)+
...
'''
tm1 <- tm_shape(NYCSubBoroughs) + 
  tm_polygons(col = NA, alpha = 0.5, border.col = "#a6bddb")+
  tm_legend(show=FALSE)+
  tm_layout(frame=FALSE)+
  tm_credits("(a)", position=c(0,0.85), size=1.5)

tm2 <- tm_shape(NYCCDistric) + 
  tm_polygons(col = NA, alpha = 0.5, border.col = "#a6bddb")+
  tm_legend(show=FALSE)+
  tm_layout(frame=FALSE)+
  tm_credits("(b)", position=c(0,0.85), size=1.5)

tm3 <- tm_shape(NYCNeighbor) + 
  tm_polygons(col = NA, alpha = 0.5, border.col = "#a6bddb")+
  tm_legend(show=FALSE)+
  tm_layout(frame=FALSE)+
  tm_credits("(c)", position=c(0,0.85), size=1.5)

legend <- tm_shape(NYCSubBoroughs) +
  tm_polygons(col = NA)+
  tm_scale_bar(position=c(0.2,0.04), text.size=0.6)+
  tm_compass(north=0, position=c(0.65,0.6))+
  tm_layout(legend.only = TRUE, legend.position=c(0.2,0.25),asp=0.1, scaleb)
...
summary(stat_by_route$per_ada)
sd(stat_by_route$per_ada)
...
####### reorder factor levels, plot with legend
stat_by_route$route_name <- reorder(stat_by_route$route_name, stat_by_route$total_stat)


ggplot(stat_by_route, aes(y = route_name)) + 
  geom_point(data = stat_by_route, aes(x = total_stat, color = "Total stations"), size = 3) +
  geom_point(data = stat_by_route, aes(x = num_ada, color = "ADA stations"), size = 3) +
  geom_dumbbell(aes(x = num_ada, xend = total_stat), size=2.5, color="#e3e2e1", 
                colour_x = "#56B4E9", colour_xend = "#E69F00",
                dot_guide=TRUE, dot_guide_size=0.25) +
  xlab("Route name") +
  ylab("Number of stations") +
  theme_bw() +
  scale_color_manual(name = "", values = c("#56B4E9", "#E69F00") )+
  ggtitle("Total number of stations vs number of ADA-accessible stations per route")
...
NYCSubBoroughs_cate %>% 
  ggplot() +
  geom_sf(aes(fill = subb_group), color = "white") +
  scale_fill_manual(values = c("#2c7fb8", "#7fcdbb", "#edf8b1"), name = "Sub-Borough Type") + 
  # overlay subway:
  #geom_sf(data = sub_nyc_crs, color = "white", fill = "black", size = 2, pch = 21) +
  theme(axis.text.x = element_blank(), axis.text.y = element_blank()) +
  theme_bw()+
  ggtitle("Sub-Borough ADA Station Access")


# Map with stations ada:
...
NYCSubBoroughs_cate %>% 
  ...
  # overlay subway:
  geom_sf(data = sub_nyc_crs, aes(color = ada), size = 1.5) +
  #geom_sf(data = ada_nyc_crs, color = "grey90", fill = "#fdb863", size = 2, pch = 21) +
  theme(axis.text.x = element_blank(), axis.text.y = element_blank()) +
  theme_bw()+
  labs(color= " ") +
  scale_color_hue(labels = c("Ordinary Stations", "ADA Stations"))+
  
  
  ggtitle("ADA station distribution")
...
tm_shape(NYCSubBoroughs_cate) +
  ...
  tm_shape(sub_nyc_crs)+
  tm_dots(col = "#7fcdbb", size = 0.5)
...
NYCCD_cate %>% 
  ggplot() +
  geom_sf(aes(fill = cd_group), color = "white") +
  ...
  theme(axis.text.x = element_blank(), axis.text.y = element_blank()) +
  theme_bw()+
  ggtitle("Community District ADA Station Access")
...
# group by and summarize station-route counts by nbh:
nbh_stat_count <- nyc_nbh_subw %>%
  ...
  # add in nbh with no subway stations
  bind_rows(
    NYCNeighbor %>% 
      as_tibble() %>% 
...
NYCNeighbor_cate %>% 
  ggplot() +
  geom_sf(aes(fill = nbh_group), color = "white") +
  scale_fill_manual(values = c("#2c7fb8", "#7fcdbb", "#edf8b1"), name = "Neighborhood Type") + 
  # overlay subway:
  #geom_sf(data = sub_nyc_crs, color = "grey80", fill = "black", size = 2, pch = 21) +
  theme(axis.text.x = element_blank(), axis.text.y = element_blank()) +
  theme_bw()+
  ggtitle("Neighborhood ADA Station Access")
...
tm5 <- tm_shape(NYCSubBoroughs_cate) +
  tm_polygons(col = "subb_group", alpha = 0.8, border.col = '#a6bddb')+

  tm_layout(frame=FALSE)+
  tm_layout(legend.show = FALSE)+
  tm_credits("(a) SBA", position=c(0,0.85), size=1.5)

tm6 <- tm_shape(NYCCD_cate) +
  tm_polygons(col = "cd_group", alpha = 0.8, border.col = '#a6bddb') +
  tm_layout(frame=FALSE)+
  tm_layout(legend.show = FALSE)+
  tm_credits("(b) Community District", position=c(0,0.85), size=1.5)

tm7 <- tm_shape(NYCNeighbor_cate) +
  tm_polygons(col = "nbh_group", alpha = 0.8, border.col = '#a6bddb') +
  tm_layout(frame=FALSE)+
  tm_layout(legend.show = FALSE)+
  tm_credits("(b) NTA", position=c(0,0.85), size=1.5)

legend <- tm_shape(NYCSubBoroughs_cate) +
  tm_polygons("subb_group",
...
t3=tmap_arrange(tm5, tm7, legend, ncol = 3)
...
summary(subb_pop_ada_den$pop_disabled_pct)
...
#subb pop den 2010
#join population related columns to subb shapefile
subb_pop_ada_10 <- subb_ada_den_joined %>%
  left_join(subb_pop_2010, by = "bor_subb")

subb_pop_ada_10 <- subb_pop_ada_10 %>%
  #calculate area
  mutate(area=st_area(.))%>%
  #then density of the population per subb
  mutate(pop_density=pop_num/area)

tm_shape(subb_pop_ada_10) +
  tm_polygons("pop_density",
              style="jenks",
              palette="PuOr",
              midpoint=NA,
...
summary(subb_pop_ada_10$pop_disabled_pct)
...
breaks = c(0, 0.0008, 0.0016, 0.0024, 0.0032, 0.0040) 

tm1 <- tm_shape(subb_pop_ada_10) +
  tm_polygons("pop_density",
              breaks = breaks,
              palette="PuBu" )+
  tm_layout(frame=FALSE)+
  tm_layout(legend.show = FALSE)+
  tm_credits("(a) 2010", position=c(0,0.85), size=1.5)

tm2 <- tm_shape(subb_pop_ada_den) +
  tm_polygons("pop_density",
              breaks = breaks,
              palette="PuBu" )+
  tm_layout(frame=FALSE)+
  tm_layout(legend.show = FALSE)+
  tm_credits("(b) 2018", position=c(0,0.85), size=1.5)

legend <- tm_shape(subb_pop_ada_10) +
  tm_polygons("pop_density",
              breaks = breaks,
              palette="PuBu",
...
t=tmap_arrange(tm1, tm2, legend, ncol = 3)

t

breaks2 = c(0, 0.0001, 0.0002, 0.0003, 0.0004, 0.0005)

tm3 <- tm_shape(subb_pop_ada_10) +
  tm_polygons("d_density",
              breaks = breaks2,
              palette="PuBu" )+
  tm_layout(frame=FALSE)+
...
legend <- tm_shape(subb_pop_ada_10) +
  tm_polygons("d_density",
              breaks = breaks2,
              palette="PuBu",
              title = "Disabled Population Density") +
  tm_scale_bar(position=c(0.2,0.04), text.size=0.6)+
  tm_compass(north=0, position=c(0.25,0.6))+
  tm_layout(legend.only = TRUE, legend.position=c(0.2,0.25),asp=0.1)


t2=tmap_arrange(tm3, tm4, legend, ncol = 3)
